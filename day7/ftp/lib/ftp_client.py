...
class ftp_client:
    ...
    def put(self,inp):
        '''
        上传文件函数
        :param cmd: 上传命令，如，put 1.txt
        :return:
        '''

        cmd,filename = inp
        if filename in os.listdir(settings.data_path):
            self.request(inp)
            md5 = hashlib.md5()
            head = {}

            head['data-size'] = os.path.getsize(os.path.join(settings.data_path,filename))
            ...
            progress.__next__()
            f = open(os.path.join(settings.data_path,filename),'rb')
    ...
    def progress_bar(self,total_size,current_percent=0,last_percent=0):
        ...
        :param current_percent:当前接收大小
        :param last_percent:上次接收大小
        :return:
        '''
        while True:
    ...
    def get(self,inp):
        '''
        下载文件函数
        :param cmd: 下载命令，ru get 1.txt
        :return:
        '''
        cmd,filename = inp
        if filename in os.listdir(settings.data_path):      # 检测本地文件是否已有同名文件
            print('您本地路径已经存在此文件')
        else:
            self.request(inp)
            ...
            if len(struct_head_bytes_len) == 4:              # 当长度是4一定是返回的数据包
                ...
                if md5.hexdigest() == msg_md5:
                    ...
                    while len(res) < msg_size:
                        ...
                        f.close()
                        print('success')
                        os.rename('%s.download'%file_path,os.path.join(settings.data_path,filename))
            ...
            while True:
                ...
                if not inp:continue
                    ...
                    if inp >= 0 and inp <= index:
                        ...
                        self.re_get(self.shelveobj[choice_file])
    ...
    def re_get(self,file):
        ...
        if len(struct_head_bytes_len) == 4:              # 当长度是4一定是返回的数据包
            ...
            if file_md5 == msg_md5 and total_size == msg_size:  # 验证是否是同一个文件
                file_path = os.path.join(settings.data_path,'%s.download'%filename)
                file_path = file_path.replace('\\','/')
                self.shelveobj[file_path] = [recevied_size,msg_size,msg_md5,'%s.download'%filename]
                        # 文件路径             已传输大小 总大小   md5   文件名
                ...
                # 废弃的数据，因为已经有了
                scrap_data = self.client.recv(recevied_size)

                progress = self.progress_bar(msg_size,recevied_size,recevied_size)
                progress.__next__()
                new_recv_size = recevied_size   # 新的已接收的大小

                # 循环接收数据，并打印进度条
                while new_recv_size < msg_size:
                    cont = self.client.recv(self.MAX_PAKET)
                    f.write(cont)
                    new_recv_size += len(cont)
                    f.flush()
                    progress.send(new_recv_size)
                    print('totaldata',msg_size)
                else:
                    f.close()
                    print('success')
                    self.logobj.info('re_get file from server %s'%str(self.host))
                    os.rename(file_path,os.path.join(settings.data_path,filename))
                    key_file_path = file_path.replace('.download','')
                    del self.shelveobj[key_file_path]
    ...
    def user_help(self):
        ...
        mkdir dir_name      -- create document as the current directory from server
        cd  dir_name        -- change curent directory as dir_name from server
        '''
        print(d)
    ...
    def run(self):
        ...
        :return:
        '''
        self.client = self.create_ftp_cilent()
        while True:
